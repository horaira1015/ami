...
public class OptionMenu implements Initializable {
	...
	public void dProceed() throws SQLException {
		...
		String query = "UPDATE `aronotun`.`information` SET `Balance` = '" + total + "' WHERE (`Id` = '" + LoginPage.id + "');";
		Statement st = Main.con.createStatement();
		int i = st.executeUpdate(query);
		if (i == 1) {
			String getBalance = "SELECT Balance FROM aronotun.information where id=" + LoginPage.id + ";";
		...
		new Main().changeScene("trans.fxml", "Transfer Message", 500, 300);
	...
	public void transferMoney() throws SQLException {
		...
		String query1 = "UPDATE `aronotun`.`information` SET `Balance` = `Balance` - " + amount + " WHERE (`Id` = '" + LoginPage.id + "');";
		String query2 = "UPDATE `aronotun`.`information` SET `Balance` = `Balance` + " + amount + " WHERE (`Id` = '" + payeeId + "');";
		Statement st = Main.con.createStatement();
		st.addBatch(query1);
		st.addBatch(query2);
	...
	public void update() throws SQLException {
		Statement st = Main.con.createStatement();
		String query = "UPDATE `aronotun`.`information` SET `First_name` = '" + fname.getText() + "', `Middle_name` = '" + mname.getText() + "', `Last_name` = '" + lname.getText() + "', `Phone_no` = '" + phoneno.getText() + "', `Email` = '" + email.getText() + "', `DOB` = '" + dob.getValue().toString() + "' WHERE (`Id` = '" + id.getText() + "');";
		int i = st.executeUpdate(query);
	...
	//sign out
	public void signOut() {
		new Main().changeScene("Startscreen.fxml", "Start Screen", 759, 484);
	}
}
