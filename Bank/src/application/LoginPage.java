...
public class LoginPage {
	...
	public void signIn(){
		try {
		String query1="Select id,password from aronotun.login where username='"+username.getText()+"';";
		...
		if(dbpassword.contentEquals(password.getText())) {
			...
			id=rs.getInt(1);
			new Main().changeScene("Success.fxml","Success",400,300);
			try{
				Thread.sleep(2000);
			}
			catch (Exception e){}
	...
	static public Customer downloadObj(int id) throws SQLException{
		String query2="Select * from aronotun.information where id='"+id+"';";
		Statement st=Main.con.createStatement();
